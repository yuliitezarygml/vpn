...
class Actions(FlaskView):
        ...
        return [
            {
                "ip": str(ip),
                "version": ip.version,
                "is_server_ip": ip in myips,
            }
            for ip in hutils.network.get_domain_ips(domain)
        ]
    ...
    def update_usage(self):
        ...
        return render_template("result.html", out_type="info", out_msg=f'<pre class="ltr" style="color:{color};">{json.dumps(usage.update_local_usage(), indent=2)}</pre>', log_file_url=None)
