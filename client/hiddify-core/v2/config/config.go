...
func setRoutingOptions(options *option.Options, opt *HiddifyOptions) {
	...
	if opt.Region != "other" {
		...
		// Prefer local rule-sets (copied next to the working dir) so connect
		// does not depend on raw.githubusercontent.com being reachable.
		geoipPath := "geoip-" + opt.Region + ".srs"
		geositePath := "geosite-" + opt.Region + ".srs"
